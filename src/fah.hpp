...
uint32_t get_hash(const char* filename)
{
    ...
    int pos = 0;
    while (*filename) {
        reinterpret_cast<char*>(&hash)[pos] += tolower(*filename);
        pos = (pos + 1) & 3;
        filename++;
    }
    return hash;
...
void create_csv_list(const char *data_fai, const char *data_fab, const char *output_csv)
{
    ...
    out << "Filename,Offset (FAB),Size (Hex),Position (FAI),Hash (Hex)" << std::endl;
    ...
    for (size_t i = 0; i < fai_entries.size(); ++i)
    {
        ...
        out << "\"" << filename << "\","
            ...
            << "0x" << std::hex << fai_position << ","
            << "0x" << std::hex << entry.hash << std::dec << std::endl;
    }
...
void import_to_archive(const char* data_fai, const char* data_fab, const char* input_dir)
{
    std::fstream fai_file(data_fai, std::ios::in | std::ios::out | std::ios::binary);
    std::fstream fab_file(data_fab, std::ios::in | std::ios::out | std::ios::binary);
    if (!fai_file || !fab_file)
    {
        std::cerr << "Failed to open files" << std::endl;
        return;
    ...
    for (const auto& entry : std::filesystem::recursive_directory_iterator(input_dir))
    {
        if (entry.is_regular_file())
        {
            ...
            uint32_t hash = get_hash(relative_path.c_str());

            // Find entry by hash and filename in FAI
            auto it = std::find_if(fai_entries.begin(), fai_entries.end(), [hash, &relative_path, &names_data, &fai_header](const fai_entry_t& entry) {
                char filename[256];
                if (fai_header.version < FAH_VER_2_00) {
                    strcpy(filename, names_data.data() + entry.name_offset);
            ...
            if (it != fai_entries.end())
            {
                // Found matching entry
                std::ifstream in(entry.path(), std::ios::binary);
                std::vector<char> data((std::istreambuf_iterator<char>(in)), std::istreambuf_iterator<char>());
                
                // Check if new file is too big
                size_t size;
                if(fai_header.version < FAH_VER_2_00) {
                    size = it->size;
                } else {
                    size = (it->size + 0x7FF) & ~0x7FF;
                }
                if (data.size() > size)
                {
                    std::cerr << "File is too big to import: " << relative_path << " Original size: " << std::hex << it->size
                              << " New size: " << data.size() << std::dec << std::endl;
                    continue;
                }

                std::cout << "Importing " << relative_path << " to offset 0x" << std::hex << it->offset << std::dec << std::endl;

                // Write the data to the FAB file
                fab_file.seekp(it->offset);
                fab_file.write(data.data(), data.size());

                // Update the FAI entry with the new size
                it->size = data.size();
            }
            else
            {
                std::cout << "No matching entry found for file: " << relative_path << std::endl;
            }
        }
    }

    // Update the FAI file with the new entry sizes
    fai_file.seekp(sizeof(fai_header_t));
    fai_file.write(reinterpret_cast<char *>(fai_entries.data()), sizeof(fai_entry_t) * fai_entries.size());
